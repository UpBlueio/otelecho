	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if path := c.Path(); path != "" {
				...
				//SJOERD: changed these line from path to path2,
				//because normaly HTTPTargetKey is request.URL.Path, but is reused below
				rAttr := semconv.HTTPRoute(path2)
				opts = append(opts, oteltrace.WithAttributes(rAttr))
				//SJOERD: only added this two lines, to override the transaction name in Sentry
				rAttr2 := semconv.HTTPTargetKey.String(path)
				opts = append(opts, oteltrace.WithAttributes(rAttr2))
			}
